...
module TemporaryEmailUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TemporaryEmailResponse.new({})
    ...
    if fetch_err
      ...
      response = TemporaryEmailResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TemporaryEmailResponse.new(fetched)
    else
