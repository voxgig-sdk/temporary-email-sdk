...
// TemporaryEmail SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class TemporaryEmailMakeError
{
    public static function call(?TemporaryEmailContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TemporaryEmailContext([], null);
        }
        $op = $ctx->op ?? new TemporaryEmailOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TemporaryEmailResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TemporaryEmailError) ? $err->msg : (string)$err;
        $msg = "TemporaryEmailSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TemporaryEmailError('', $msg, $ctx);
        ...
        if ($err instanceof TemporaryEmailError) {
