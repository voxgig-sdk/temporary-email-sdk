...
// TemporaryEmail SDK feature test
//
...
class FtCtrl extends TemporaryEmailControl
{
...
class FtClient
{
    ...
    private TemporaryEmailUtility $utility;

    public function __construct(TemporaryEmailUtility $utility, array $options)
    {
    ...
    public function get_utility(): TemporaryEmailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TemporaryEmailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TemporaryEmailContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new TemporaryEmailUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (TemporaryEmailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TemporaryEmailConfig::make_feature($name);
    ...
    private static function buildUrl(TemporaryEmailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TemporaryEmailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TemporaryEmailResponse($fetched);
        }
    ...
    private function fail(TemporaryEmailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TemporaryEmailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
