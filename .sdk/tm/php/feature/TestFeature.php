...
// TemporaryEmail SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TemporaryEmailTestFeature extends TemporaryEmailBaseFeature
{
    ...
    public function init(TemporaryEmailContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TemporaryEmailContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TemporaryEmailParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TemporaryEmailContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TemporaryEmailContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TemporaryEmailParam::call($ctx, $k);
