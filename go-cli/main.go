// Command TemporaryEmail-cli is a CLI + REPL for the TemporaryEmail SDK,
...
const prompt = "temporary-email"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "email inbox message"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMPORARY_EMAIL_APIKEY (api key), TEMPORARY_EMAIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMPORARY_EMAIL_APIKEY carries the API key and
	// TEMPORARY_EMAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMPORARY_EMAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMPORARY_EMAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTemporaryEmailSDK(opts)
