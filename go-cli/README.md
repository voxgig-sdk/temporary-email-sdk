...
boru-driven command-line client **and** interactive REPL for the TemporaryEmail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-cli)
make build

# 2. See usage (words, entities, env vars)
./temporary-email-cli --help

# 3. Provide credentials once, via the environment
export TEMPORARY_EMAIL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./temporary-email-cli load 1 email            # {id:1} shorthand
./temporary-email-cli load '{id:1}' email       # explicit match map

# 5. Override the API base URL for a single call
TEMPORARY_EMAIL_BASE=https://api.example.com ./temporary-email-cli load 1 email

# 6. No arguments -> interactive REPL
./temporary-email-cli
temporary-email> load 1 email
temporary-email> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/temporary-email-cli load 1 email
   ```
...
```sh
./temporary-email-cli load 1 email          # scalar shorthand for {id:1}
./temporary-email-cli load '{id:1}' email     # explicit match map
```
...
./temporary-email-cli load 1 email
```
...
Run with no arguments to open a REPL (prompt `temporary-email>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `email`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temporary-email-cli`. |
...
(`./temporary-email-cli <expr>`) and interactively (the REPL), and expressions compose the
