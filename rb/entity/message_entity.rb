# TemporaryEmail SDK Message entity
...
class MessageEntity
  ...
  def data_set(args)
    if args
      @_data = TemporaryEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TemporaryEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Message, Hash] the loaded Message; raises TemporaryEmailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TemporaryEmailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
