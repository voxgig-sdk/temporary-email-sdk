...
module TemporaryEmailUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = TemporaryEmailContext.new({}, nil)
    end
    op = ctx.op || TemporaryEmailOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || TemporaryEmailResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(TemporaryEmailError) ? err.msg : err.to_s
    ...
    sdk_err = TemporaryEmailError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(TemporaryEmailError)
