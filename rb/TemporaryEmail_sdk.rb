...
require_relative 'TemporaryEmail_types'
...
class TemporaryEmailSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TemporaryEmailHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TemporaryEmailHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TemporaryEmailFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TemporaryEmailUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TemporaryEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TemporaryEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TemporaryEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TemporaryEmailError => err
    ...
    if fetched.is_a?(Hash)
      status = TemporaryEmailHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TemporaryEmailError.new(
        "graphql_error", "TemporaryEmailSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TemporaryEmailSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
