...
local TemporaryEmailSDK = {}
TemporaryEmailSDK.__index = TemporaryEmailSDK
...
TemporaryEmailSDK._make_feature = _make_feature


function TemporaryEmailSDK.new(options)
  local self = setmetatable({}, TemporaryEmailSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TemporaryEmailSDK:options_map()
...
function TemporaryEmailSDK:get_utility()
  return Utility.copy(self._utility)
end


function TemporaryEmailSDK:get_root_ctx()
  return self._rootctx
end


function TemporaryEmailSDK:prepare(fetchargs)
...
function TemporaryEmailSDK:direct(fetchargs)
...
-- by passing it.
function TemporaryEmailSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TemporaryEmailSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TemporaryEmailSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TemporaryEmailSDK:Email(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TemporaryEmailSDK:Inbox(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TemporaryEmailSDK:Message(data)
...
function TemporaryEmailSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TemporaryEmailSDK.new(sdkopts)
  sdk.mode = "test"
