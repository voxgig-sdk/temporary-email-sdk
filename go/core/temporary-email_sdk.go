...
import (
	...
	vs "github.com/voxgig-sdk/temporary-email-sdk/go/utility/struct"
)

type TemporaryEmailSDK struct {
	Mode     string
...
func (sdk *TemporaryEmailSDK) OptionsMap() map[string]any {
...
func (sdk *TemporaryEmailSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *TemporaryEmailSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *TemporaryEmailSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TemporaryEmailSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *TemporaryEmailSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *TemporaryEmailSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("TemporaryEmailSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *TemporaryEmailSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TemporaryEmailSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("TemporaryEmailSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *TemporaryEmailSDK {
	...
	sdk := NewTemporaryEmailSDK(sdkopts)
	sdk.Mode = "test"
