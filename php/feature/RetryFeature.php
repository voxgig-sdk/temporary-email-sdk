...
// TemporaryEmail SDK retry feature
...
class TemporaryEmailRetryFeature extends TemporaryEmailBaseFeature
{
    ...
    public function init(TemporaryEmailContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TemporaryEmailContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TemporaryEmailContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TemporaryEmailContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
