...
// TemporaryEmail SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TemporaryEmailOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TemporaryEmailOmni::structfix($now, Struct::undef()))
                        !== json_encode(TemporaryEmailOmni::structfix($val, Struct::undef()))) {
