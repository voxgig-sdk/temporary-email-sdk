...
// TemporaryEmail SDK pipeline test
//
...
{
    private static function utility(): TemporaryEmailUtility
    {
        return new TemporaryEmailUtility();
    }
    ...
    private static function ctx(array $over = []): TemporaryEmailContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TemporaryEmailContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TemporaryEmailOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TemporaryEmailError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TemporaryEmailOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TemporaryEmailMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TemporaryEmailOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TemporaryEmailMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TemporaryEmailOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TemporaryEmailMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TemporaryEmailSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TemporaryEmailMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TemporaryEmailResponse([]),
            'result' => new TemporaryEmailResult([]),
        ]);
        [, $err] = TemporaryEmailMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec([]),
            'response' => null,
            'result' => new TemporaryEmailResult([]),
        ]);
        [, $err] = TemporaryEmailMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec(['step' => 's']),
            'response' => new TemporaryEmailResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TemporaryEmailResult([]),
        ]);
        [, $err] = TemporaryEmailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec(['step' => 's']),
            'response' => new TemporaryEmailResponse(self::resp(200, ['v' => 1])),
            'result' => new TemporaryEmailResult([]),
        ]);
        [, $err] = TemporaryEmailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec(['step' => 's']),
            'response' => new TemporaryEmailResponse(self::resp(200, ['v' => 2])),
            'result' => new TemporaryEmailResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TemporaryEmailMakeResponse::call($ctx);
    ...
    {
        $preset = new TemporaryEmailResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec([]),
            'response' => new TemporaryEmailResponse([]),
            'result' => new TemporaryEmailResult([]),
        ...
        [$got, $err] = TemporaryEmailMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TemporaryEmailResult([])]);
        [, $err] = TemporaryEmailMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TemporaryEmailSpec([]), 'result' => null]);
        [, $err] = TemporaryEmailMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TemporaryEmailSpec(['step' => 's']),
            'result' => new TemporaryEmailResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TemporaryEmailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TemporaryEmailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TemporaryEmailSpec(['step' => 's']),
            'result' => new TemporaryEmailResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TemporaryEmailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TemporaryEmailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TemporaryEmailResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TemporaryEmailSpec([]),
            'result' => new TemporaryEmailResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TemporaryEmailMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TemporaryEmailMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TemporaryEmailError('boom', 'boom');
        $utility->fetcher = function (TemporaryEmailContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TemporaryEmailSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TemporaryEmailMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TemporaryEmailContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TemporaryEmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TemporaryEmailResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        TemporaryEmailMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TemporaryEmailContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TemporaryEmailSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TemporaryEmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TemporaryEmailResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TemporaryEmailSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TemporaryEmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TemporaryEmailMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TemporaryEmailMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TemporaryEmailResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TemporaryEmailDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TemporaryEmailResult(['ok' => false])]);
        $this->expectException(TemporaryEmailError::class);
        TemporaryEmailDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TemporaryEmailResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TemporaryEmailMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TemporaryEmailMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TemporaryEmailBaseFeature();
        $b = new TemporaryEmailBaseFeature();
        TemporaryEmailFeatureAdd::call($ctx, $a);
        TemporaryEmailFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TemporaryEmailBaseFeature
    {
        $f = new TemporaryEmailBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TemporaryEmailFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TemporaryEmailContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TemporaryEmailSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TemporaryEmailPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        TemporaryEmailPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TemporaryEmailResponse(['headers' => null]),
            'result' => new TemporaryEmailResult([]),
        ]);
        TemporaryEmailResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        TemporaryEmailResultBody::call($ctx);
