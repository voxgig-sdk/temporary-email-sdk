...
[MCP](https://modelcontextprotocol.io) server exposing the TemporaryEmail SDK as
two agent tools — `temporary-email_list` and `temporary-email_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-mcp)
make build

# 2. Provide credentials via the environment
export TEMPORARY_EMAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temporary-email \
  -- /absolute/path/to/temporary-email-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temporary-email-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temporary-email_list: first page of records
{ "entity": "email" }
{ "entity": "email", "query": { } }

// temporary-email_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temporary-email \
     -- "$PWD"/dist/*/temporary-email-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temporary-email_list` and `temporary-email_load` tools now appear
   in new sessions. Ask the agent to *"list email using temporary-email"*
   and it calls `temporary-email_list` with `{"entity":"email"}`.
...
```sh
export TEMPORARY_EMAIL_APIKEY=sk_live_xxx            # API key
export TEMPORARY_EMAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `temporary-email_list` tool
...
### Call the `temporary-email_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `temporary-email_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temporary-email_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMPORARY_EMAIL_APIKEY` | API key sent with every request. |
| `TEMPORARY_EMAIL_BASE` | Optional override of the API base URL. |
...
```sh
./temporary-email-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temporary-email_load","arguments":{"entity":"email","query":{"id":1}}}}'
```
