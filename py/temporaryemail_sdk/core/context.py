...
from temporaryemail_sdk.core.helpers import get_ctx_prop, to_map
...
class TemporaryEmailContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = TemporaryEmailControl()
        ...
        if isinstance(sp, TemporaryEmailSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, TemporaryEmailResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, TemporaryEmailResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return TemporaryEmailOperation({})
    ...
    def make_error(self, code, msg):
        return TemporaryEmailError(code, msg, self)
