# TemporaryEmail SDK test runner
...
class TemporaryEmailTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TemporaryEmailTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TemporaryEmailTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TemporaryEmailTestRunner.getenv("TEMPORARY_EMAIL_TEST_LIVE")
        override = TemporaryEmailTestRunner.getenv("TEMPORARY_EMAIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TemporaryEmailTestRunner.getenv(key)
        ...
        explain = TemporaryEmailTestRunner.getenv("TEMPORARY_EMAIL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TEMPORARY_EMAIL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TemporaryEmailTestRunner._test_control is not None:
            return TemporaryEmailTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TemporaryEmailTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TemporaryEmailTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TemporaryEmailTestRunner.load_test_control()
...
def load_env_local():
    TemporaryEmailTestRunner.load_env_local()


def env_override(m):
    return TemporaryEmailTestRunner.env_override(m)


def entity_data(v):
    return TemporaryEmailTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TemporaryEmailTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TemporaryEmailTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TemporaryEmailTestRunner.load_test_control()


def live_client_options():
    return TemporaryEmailTestRunner.live_client_options()


def live_delay_ms():
    return TemporaryEmailTestRunner.live_delay_ms()
