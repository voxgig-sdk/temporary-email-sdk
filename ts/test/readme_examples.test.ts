...
//     (\`new TemporaryEmailSDK(...)\` / \`TemporaryEmailSDK.test(...)\` become
//     \`TemporaryEmailSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TemporaryEmailSDK'
...
const TEST_SEED = {"entity":{"email":{"test01":{"id":"test01"}},"inbox":{"test01":{"id":"test01"}},"message":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TemporaryEmailSDK(...)\` and any
// \`TemporaryEmailSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TemporaryEmailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TemporaryEmailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
